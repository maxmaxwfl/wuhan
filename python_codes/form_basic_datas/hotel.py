...
#ck
"""
sql_query = "SELECT ZHENGJIANHM,XINGMING,RUZHUSJ,TUIFANGSJ,QIYEMC,YEWULB FROM GATOJZ.T_GNLK_ADD  WHERE ZHENGJIANHM IN %s OR ZHENGJIANHM IN %s OR ZHENGJIANHM IN %s"%(ck_sfz_list[0],ck_sfz_list[1],ck_sfz_list[2])
ck_df = pd.read_sql(sql_query,con=connection)
ck_df = ck_df.sort_values(by=["ZHENGJIANHM"])
ck_df.columns = ['GMSFHM','XM','RZSJ','TFSJ','QYMC','YWLB']
col_list = ['GMSFHM','XM','QYMC','YWLB']
railn = {c:types.VARCHAR(ck_df[c].str.len().max())for c in col_list}
ck_df.to_sql("CK_HOTEL",tj_con,if_exists='append',index=False,dtype=railn)
"""
#xd
sql_query = "SELECT ZHENGJIANHM,XINGMING,RUZHUSJ,TUIFANGSJ,QIYEMC,YEWULB FROM GATOJZ.T_GNLK_ADD  WHERE ZHENGJIANHM IN %s OR ZHENGJIANHM IN %s OR ZHENGJIANHM IN %s"%(xd_sfz_list[0],xd_sfz_list[1],xd_sfz_list[2])
xd_df = pd.read_sql(sql_query,con=connection)
xd_df = xd_df.sort_values(by=["ZHENGJIANHM"])
xd_df.columns = ['GMSFHM','XM','RZSJ','TFSJ','QYMC','YWLB']
col_list = ['GMSFHM','XM','QYMC','YWLB']
railn = {c:types.VARCHAR(xd_df[c].str.len().max())for c in col_list}
xd_df.to_sql("XD_HOTEL",tj_con,if_exists='append',index=False,dtype=railn)